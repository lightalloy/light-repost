...
### VPS + Caddy + webhook (краткий план)

**Инфра**
- Одна **shared VPS** (потом можно накрутить ещё ботов/приложения на те же 80/443).
- VPS лучше **не в РФ**: боту нужно стабильно ходить на `api.telegram.org` (исходящее). Входящий webhook на РФ иногда ок, исходящий API — часто нет. Перед выбором хостера: `curl -I https://api.telegram.org`.
- **Caddy на хосте** (без Docker): TLS/Let's Encrypt сам, `reverse_proxy` на порт бота. Удобно, когда приложений станет несколько.
- **Бот в контейнере**: наружу только `127.0.0.1:8080` (или другой порт), Caddy проксирует `https://repost.домен → localhost:8080`.
- Домен: A-запись на IP VPS (когда будет).

**Схема**

```text
Internet → :443 Caddy (хост, TLS)
              → 127.0.0.1:8080 bot container (aiogram webhook)
```

**Шаги**
1. Поднять VPS, проверить доступ к Telegram API и VK.
2. Поставить Docker; задеплоить бота (сначала можно ещё polling, чтобы проверить TG↔VK с сервера).
3. Домен → DNS; поставить Caddy; сайт с TLS открывается.
4. В коде: режим webhook (`set_webhook` + aiohttp), `WEBHOOK_URL` / `WEBHOOK_SECRET` / `PORT` в `.env`; хендлеры те же.
5. Бот слушает порт на localhost; Caddyfile → proxy; выключить polling (два режима одновременно нельзя).
6. Проверка: `getWebhookInfo`, `/start`, пост в канал → VK.
7. Позже: новые vhost’ы в том же Caddy на другие контейнеры.

**Не сейчас:** Caddy в compose бота; отдельная VPS только под бота; k8s.
...
VPS (лучше EU) → Docker-бот → Caddy на хосте + домен → переключение на webhook.
